...
/*   Updated: 2024/06/28 11:53:05 by jberdugo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_check_flags	check_all_textures(t_list *file)
{
	...
	while (file)
	{
		...
		if (ft_strncmp(tmp, "NO ", 3) && (mask & F_NORTH) != F_NORTH)
			mask |= F_NORTH;
		else if (ft_strncmp(tmp, "SO ", 3) && (mask & F_SOUTH) != F_SOUTH)
			mask |= F_SOUTH;
		else if (ft_strncmp(tmp, "WE ", 3) && (mask & F_WEST) != F_WEST)
			mask |= F_WEST;
		else if (ft_strncmp(tmp, "EA ", 3) && (mask & F_EAST) != F_EAST)
			mask |= F_EAST;
		else if (ft_strncmp(tmp, "F ", 2) && (mask & F_FLOOR) != F_FLOOR)
			mask |= F_FLOOR;
		else if (ft_strncmp(tmp, "C ", 2) && (mask & F_CEILING) != F_CEILING)
			mask |= F_CEILING;
		else if (ft_strncmp(tmp, "NO ", 3) || ft_strncmp(tmp, "SO ", 3)
			|| ft_strncmp(tmp, "WE ", 3) || ft_strncmp(tmp, "EA ", 3)
			|| ft_strncmp(tmp, "F ", 3) || ft_strncmp(tmp, "C ", 3))
			mask |= FAILURE | REPEATED;
		file = file->next;
	}
